...
P0 = diag([0.02 0.001]);        %<--- (baja)va a variar
X0 = [0.3; -0.4];               %<--- lo dejamos constante.
...
%% c) alpha = {10 1 0.1 0.01 0.001 0.0001}

for alpha = [10 1 0.1 0.01 0.001 0.0001]
    Rd = sigmaW2;
    
    [~, ~, Ks] = Kalman( yk, alpha*P0, Qd, X0, Fd, Gd ,Hd, Rd);

 figure(1)
    subplot(1, 2, 1)
    hold on
    plot(Ks(1, :), 'LineWidth', 2, 'DisplayName', "\alpha = "+alpha);
    hold off
    subplot(1, 2, 2)
    hold on
    plot(Ks(2, :), 'LineWidth', 2, 'DisplayName', "\alpha = "+alpha);
    hold off
end
    ...
    legend show
    ylim([-0.01, 0.17])
    title("Ganancias del FK variando \alpha. P0")
